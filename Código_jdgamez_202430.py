...
def main():
    ...
    def impedancia_propia(index):
        radio_medio_geometrico = (float(line[index][2]) / 2) * np.exp(-1 / 4)
        return (
            (4 * np.pi * 1e-7)
            / (2 * np.pi)
            * np.log(float(line[index][0]) * 2 / radio_medio_geometrico)
        )

    def impedancia_mutua(index1, index2):
        distancia_sombra = np.sqrt(
        ...
        return (
            (4 * np.pi * 1e-7)
            / (2 * np.pi)
            * np.log(distancia_sombra / distancia_conductor)
        )
    ...
    def campo_electrico_horizontal_conductor(
        carga_conductor,
        punto_horizontal,
        punto_vertical,
        ...
        return (carga_conductor * (punto_horizontal - distancia_horizontal_fase)) / (
            (2 * np.pi * 8.8541878188 * 1e-12)
            * (
                (punto_horizontal - distancia_horizontal_fase) ** 2
                + (distancia_vertical_fase - punto_vertical) ** 2
            )
        ) - (carga_conductor * (punto_horizontal - distancia_horizontal_fase)) / (
            (2 * np.pi * 8.8541878188 * 1e-12)
            * (
                (punto_horizontal - distancia_horizontal_fase) ** 2
                + (distancia_vertical_fase + punto_vertical) ** 2
    ...
    def campo_electrico_vertical_conductor(
        carga_conductor,
        punto_horizontal,
        punto_vertical,
        distancia_horizontal_fase,
        ...
        return (carga_conductor * (punto_vertical - distancia_vertical_fase)) / (
            (2 * np.pi * 8.8541878188 * 1e-12)
            * (
                (punto_horizontal - distancia_horizontal_fase) ** 2
                + (distancia_vertical_fase - punto_vertical) ** 2
            )
        ) - (carga_conductor * (punto_vertical + distancia_vertical_fase)) / (
            (2 * np.pi * 8.8541878188 * 1e-12)
            * (
                (punto_horizontal - distancia_horizontal_fase) ** 2
                + (distancia_vertical_fase + punto_vertical) ** 2
    ...
    if st.button("Analyze"):
        ...
        impedancia_matriz = np.zeros((len(line), len(line)), dtype=complex)
        ...
        for i in range(len(line)):
            for j in range(len(line)):
                if i == j:
                    ...
                    impedancia_matriz[i, j] = impedancia_propia(i)
                else:
                    coeficientes_potencial_matriz[i, j] = coeficientes_potencial_mutuos(
                        i, j
                    )
                    impedancia_matriz[i, j] = impedancia_mutua(i, j)

        impedancia_fase_fase = impedancia_matriz[
            0 : (numero_fase_a + numero_fase_b + numero_fase_c),
            0 : (numero_fase_a + numero_fase_b + numero_fase_c),
        ]

        impedancia_fase_guarda = impedancia_matriz[
            0 : (numero_fase_a + numero_fase_b + numero_fase_c),
        ...
        impedancia_guarda_fase = impedancia_matriz[
            (numero_fase_a + numero_fase_b + numero_fase_c) : (
                numero_fase_a + numero_fase_b + numero_fase_c
            )
            + numero_guarda,
        ...
        impedancia_guarda_guarda = impedancia_matriz[
            (numero_fase_a + numero_fase_b + numero_fase_c) : (
                numero_fase_a + numero_fase_b + numero_fase_c
            )
            + numero_guarda,
        ...
        capacitancia = np.linalg.inv(coeficientes_potencial_matriz)

        punto_horizontal = np.linspace(
            -longitud_zona_incertidumbre, longitud_zona_incertidumbre, 100
        )

        distancia_horizontal_conductor = np.zeros(len(line))
        distancia_vertical_conductor = np.zeros(len(line))
        ...
        corriente_conductores_fase = np.vstack(
            [
        ...
        corriente_guarda = (
            -np.linalg.inv(impedancia_guarda_guarda)
            @ (impedancia_guarda_fase)
            @ corriente_conductores_fase
        )

        corriente = np.vstack([corriente_conductores_fase, corriente_guarda])
        ...
        for _ in range(numero_fase_a):
            ...
            campo_electrico_vertical_a.append(
                campo_electrico_vertical_conductor(
                    carga_conductores[_],
                    punto_horizontal,
                    punto_vertical,
                    distancia_horizontal_conductor[_],
        ...
        for _ in range(numero_guarda):
            campo_electrico_horizontal_guarda.append(
                campo_electrico_horizontal_conductor(
                    carga_conductores[
                        _ + numero_fase_a + numero_fase_b + numero_fase_c
                    ],
                    punto_horizontal,
                    punto_vertical,
                    distancia_horizontal_conductor[
                        _ + numero_fase_a + numero_fase_b + numero_fase_c
                    ],
                    distancia_vertical_conductor[
                        _ + numero_fase_a + numero_fase_b + numero_fase_c
                    ],
                )
            )

            campo_electrico_vertical_guarda.append(
                campo_electrico_vertical_conductor(
                    carga_conductores[
                        _ + numero_fase_a + numero_fase_b + numero_fase_c
                    ],
        ...
        campo_electrico_horizontal_guarda = np.array(campo_electrico_horizontal_guarda)
        ...
        campo_electrico_vertical_guarda = np.array(campo_electrico_vertical_guarda)

        campo_electrico_horizontal_resultante = (
            np.sum(campo_electrico_horizontal_a, axis=0)
            + np.sum(campo_electrico_horizontal_b, axis=0)
            + np.sum(campo_electrico_horizontal_c, axis=0)
            + np.sum(campo_electrico_horizontal_guarda, axis=0)
        )

        campo_electrico_vertical_resultante = (
            np.sum(campo_electrico_vertical_a, axis=0)
            + np.sum(campo_electrico_vertical_b, axis=0)
            + np.sum(campo_electrico_vertical_c, axis=0)
            + np.sum(campo_electrico_vertical_guarda, axis=0)
        )
        ...
        campo_magnetico_horizontal_guarda = []

        campo_magnetico_vertical_a = []
        campo_magnetico_vertical_b = []
        campo_magnetico_vertical_c = []
        campo_magnetico_vertical_guarda = []

        for _ in range(numero_fase_a + numero_fase_b + numero_fase_c + numero_guarda):
        ...
        for _ in range(numero_guarda):
            campo_magnetico_horizontal_guarda.append(
                campo_magnetico(
                    corriente[_ + numero_fase_a + numero_fase_b + numero_fase_c],
                    distancia_conductores_punto[
                        _ + numero_fase_a + numero_fase_b + numero_fase_c
                    ],
                    vector_phi_horizontal[
            ...
            campo_magnetico_vertical_guarda.append(
                campo_magnetico(
                    corriente[_ + numero_fase_a + numero_fase_b + numero_fase_c],
                    distancia_conductores_punto[
                        _ + numero_fase_a + numero_fase_b + numero_fase_c
        ...
        campo_magnetico_horizontal_guarda = np.array(campo_magnetico_horizontal_guarda)
        ...
        campo_magnetico_vertical_guarda = np.array(campo_magnetico_vertical_guarda)

        campo_magnetico_horizontal_resultante = (
            np.sum(campo_magnetico_horizontal_a, axis=0)
            + np.sum(campo_magnetico_horizontal_b, axis=0)
            + np.sum(campo_magnetico_horizontal_c, axis=0)
            + np.sum(campo_magnetico_horizontal_guarda, axis=0)
        )

        campo_magnetico_vertical_resultante = (
            np.sum(campo_magnetico_vertical_a, axis=0)
            + np.sum(campo_magnetico_vertical_b, axis=0)
            + np.sum(campo_magnetico_vertical_c, axis=0)
            + np.sum(campo_magnetico_vertical_guarda, axis=0)
        )
        ...
        for _ in range(numero_guarda):
            campo_magnetico_ideal_horizontal_guarda.append(
                campo_magnetico_horizontal_fase(
                    corriente[_ + numero_fase_a + numero_fase_b + numero_fase_c],
                    punto_horizontal,
                    punto_vertical,
            ...
            campo_magnetico_ideal_vertical_guarda.append(
                campo_magnetico_vertical_fase(
                    corriente[_ + numero_fase_a + numero_fase_b + numero_fase_c],
                    punto_horizontal,
                    punto_vertical,
        ...
        campo_magnetico_horizontal_guarda = np.array(
            campo_magnetico_ideal_horizontal_guarda
        )
        ...
        campo_magnetico_ideal_vertical_guarda = np.array(
            campo_magnetico_ideal_vertical_guarda
        )

        campo_magnetico_ideal_horizontal_resultante = (
            np.sum(campo_magnetico_ideal_horizontal_a, axis=0)
            + np.sum(campo_magnetico_ideal_horizontal_b, axis=0)
            + np.sum(campo_magnetico_ideal_horizontal_c, axis=0)
            + np.sum(campo_magnetico_ideal_horizontal_guarda, axis=0)
        )

        campo_magnetico_ideal_vertical_resultante = (
            np.sum(campo_magnetico_ideal_vertical_a, axis=0)
            + np.sum(campo_magnetico_ideal_vertical_b, axis=0)
            + np.sum(campo_magnetico_ideal_vertical_c, axis=0)
            + np.sum(campo_magnetico_ideal_vertical_guarda, axis=0)
        )
        ...
        for _ in range(numero_guarda):
            campo_magnetico_real_imagenes_horizontal_guarda.append(
                campo_magnetico_horizontal_fase_imagenes(
                    corriente[_ + numero_fase_a + numero_fase_b + numero_fase_c],
                    punto_horizontal,
                    punto_vertical,
            ...
            campo_magnetico_real_imagenes_vertical_guarda.append(
                campo_magnetico_vertical_fase_imagenes(
                    corriente[_ + numero_fase_a + numero_fase_b + numero_fase_c],
                    punto_horizontal,
                    punto_vertical,
        ...
        campo_magnetico_real_imagenes_horizontal_guarda = np.array(
            campo_magnetico_real_imagenes_horizontal_guarda
        )
        ...
        campo_magnetico_real_imagenes_vertical_guarda = np.array(
            campo_magnetico_real_imagenes_vertical_guarda
        )

        campo_magnetico_real_imagenes_horizontal_resultante = (
            np.sum(campo_magnetico_real_imagenes_horizontal_a, axis=0)
            + np.sum(campo_magnetico_real_imagenes_horizontal_b, axis=0)
            + np.sum(campo_magnetico_real_imagenes_horizontal_c, axis=0)
            + np.sum(campo_magnetico_real_imagenes_horizontal_guarda, axis=0)
        )

        campo_magnetico_real_imagenes_vertical_resultante = (
            np.sum(campo_magnetico_real_imagenes_vertical_a, axis=0)
            + np.sum(campo_magnetico_real_imagenes_vertical_b, axis=0)
            + np.sum(campo_magnetico_real_imagenes_vertical_c, axis=0)
            + np.sum(campo_magnetico_real_imagenes_vertical_guarda, axis=0)
        )
